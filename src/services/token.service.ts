...
import { signAccessToken, signRefreshToken, parseDurationToMs, verifyRefreshToken } from '@/utils/jwt';
...
export const rotateRefreshToken = async (rawToken: string, req: Request) => {
  ...
  try {
    ...
    throw ApiError.unauthorized('Refresh token khong hop le hoac da het han');
  }
  ...
  if (stored.revoked) {
    // Token da bi thu hoi nhung van duoc dung lai -> co the bi danh cap.
    // Thu hoi toan bo refresh token cua user nay de phong ngua.
    await RefreshToken.updateMany({ user: stored.user, revoked: false }, { revoked: true });
    throw ApiError.unauthorized('Phat hien su dung token bat thuong, vui long dang nhap lai');
  }
  ...
  if (!user || !user.isActive) {
    throw ApiError.unauthorized('Tai khoan khong hop le');
  }
  if (user.tokenVersion !== payload.tokenVersion) {
    throw ApiError.unauthorized('Phien dang nhap da het hieu luc, vui long dang nhap lai');
  }

  // Thu hoi token cu va cap token moi (rotation)
  stored.revoked = true;
  const { accessToken, refreshToken } = await issueTokenPair(user, req);
  stored.replacedByJti = JSON.parse(Buffer.from(refreshToken.split('.')[1], 'base64').toString()).jti;
  await stored.save();
